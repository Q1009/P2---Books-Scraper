...
def extract_category_data(url : str) -> list[dict[str, str]] :
    """
    This function scraps the home page of the website in search of all the book categories available.
    Returns a list of dictionaries containing the category name and the category url.
    Requires the requests and bs4 libraries.

        Parameters:
            url (str): the string of the website's url

        Returns:
            extracted_category_data (list[dict[str, str]]): the list of dictionaries containing the name and the url of a category as strings
...
def create_directories_and_urls_file(extracted_category_data : list[dict[str, str]]) -> None :
    """
    This function creates a local parent directory named 'Books' and then a child directory per category.
    In the parent directory, creates a .txt file containing every category url.
    Requires the os library.
    ...
    # Creates a .txt file and writes every url in, then creates a child directory with error handling
    with open('Books/category_urls_output.txt', 'w') as file_txt:
        for category_data in extracted_category_data:
            ...
            nested_directory : str = parent_directory_name + '/' + category_data['category_name']
            try:
...
def extract_article_urls(url : str) -> list[str] :
    """
    This function scraps the page(s) of each category and fetches the url of every book
    Returns a list of every book's url of the category as a string
    Requires the resquests and bs4 libraries.
...
def get_article_data(name_of_category : str) -> list[dict]:
    """
    This function scraps the page of a book and extracts all required information.
    It also dowloads the book's cover as a name_of_the_book.jpg image and saves it into the appropriate directory.
    Returns a list of dictionaries containing all the required information on the book page.
    Requires the resquests, bs4, io and PIL libraries.

        Parameters:
            name_of_category (str): the book category that will be scraped

        Returns:
            extracted_article_data (list[dict]): the list of dictionaries containing all the required information on the book
            dict_keys(['product_page_url', 'title', 'review_rating', 'product_description', 'image_url', 'category', 'upc', 
            'product type', 'price (excl. tax)', 'price (incl. tax)', 'tax', 'availability', 'number of reviews'])
                
    """
    # Instance of object type list, that will contain a dictionary of data per book
    extracted_article_data: list[dict] = []
    # Opening the file.txt listing every book url in the category
    with open ('Books/' + name_of_category + '/' + name_of_category + '_book_urls.txt', 'r') as file_txt:
        for row in file_txt:
            ...
            if response.status_code == 200:
                # Instance of the dictionary that will contain all data and be appended to the list
                extracted_data : dict = {}
                # Data extraction
                ...
                # Instance of a variable to create a byte object out of the url content
                image_file = io.BytesIO(image_content)
                # Instance of a variable to convert the byte object into an RGB image
                image = Image.open(image_file).convert('RGB')
                # Saving the RGB image in the relative and appropriate directory
                file_path_name_format : str = 'Books/' + name_of_category + '/' + extracted_data['title'].replace('/','_') + '.jpg'
...
def transform_article_data(data_to_transform : list[dict]) -> list[dict]:
    """
    This function transforms the extracted book data to better suit our requirements.
    Renames the dictionary keys to prepare for csv headers, deletes special caracters or unnecessary content.
    Converts numbers from string to int or float.
    Converts the review rating from string to integer by calling another function.
    Returns a list of dictionaries containing all the required transformed information on the book page.
    Requires the re library.

        Parameters:
            data_to_transform (list[dict]): the list of dictionaries containing all the required information on the book
            dict_keys(['product_page_url', 'title', 'review_rating', 'product_description', 'image_url', 'category', 'upc', 
            'product type', 'price (excl. tax)', 'price (incl. tax)', 'tax', 'availability', 'number of reviews'])

        Returns:
            transformed_article_data (list[dict]): the list of dictionaries containing all the required transformed information of the books
            dict_keys(['product_page_url', 'universal_product_code', 'title', 'price_including_tax', 'price_excluding_tax', 
            'number_available', 'product_description', 'category', 'review_rating', 'image_url'])
                
    """
    transformed_article_data : list[dict] = []
    for data in data_to_transform:
        # Instance of a dictionary that will contain all the transformed data and then be appended to the list
        ...
        # Extraction of the digits only in this string and conversion into a integer
        ...
        # Calling a external function to convert the string rating into an integer rating
...
def load_article_data(data_to_load : list[dict], name_of_category : str) -> int:
    """
    This function reads through a list of dictionaries and creates a .csv file in the appropriate directory in order to 
    write each dictionary as a row of the .csv file.
    The header is the list of the dictionaries' keys, so they must be the same.
    Returns an integer (for debugging purposes) corresponding to the number of dictionaries whose values has been writen 
    into the .csv file.
    Requires the csv library.

        Parameters:
            data_to_load (list[dict]): the list of dictionaries containing all the required transformed information of the books
            dict_keys(['product_page_url', 'universal_product_code', 'title', 'price_including_tax', 'price_excluding_tax', 
            'number_available', 'product_description', 'category', 'review_rating', 'image_url'])
            name_of_category (str) : name of the category that is being scraped

        Returns:
            number_of_books (int) : an integer corresponding to the number of books whose data has been writen in the csv file
                
    """
    # Creates a .csv file in the appropriate directory and writes each dictionary of the list as a line
...
def load_article_urls(urls_to_load : list[str], name_of_category : str) -> None:
    """
    This function writes in a .txt file the list of each book's url. A row is equivalent to an url.
    ...
    """
    # Creates a .txt file in the appropriate directory in order to write an url on a row of the file
